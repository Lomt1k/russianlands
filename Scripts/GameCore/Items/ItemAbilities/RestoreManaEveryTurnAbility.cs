...
namespace TextGameRPG.Scripts.GameCore.Items.ItemAbilities
{
    internal class RestoreManaEveryTurnAbility : ItemAbilityBase
    {
        ...
        {
            return chanceToSuccessPercentage >= 100
                ? string.Format(Localization.Localization.Get(session, "ability_restore_mana_each_turn"), manaValue)
                : string.Format(Localization.Localization.Get(session, "ability_restore_mana_percentage_each_turn"), chanceToSuccessPercentage, manaValue);
        }
    }
}
