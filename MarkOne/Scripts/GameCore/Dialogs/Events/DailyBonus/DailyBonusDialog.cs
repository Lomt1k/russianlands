...
public class DailyBonusDialog : DialogBase
{
    ...
    {
        if (IsNewRewardAvailable(session))
        {
            await StartWithClaimNewReward().FastAwait();
        }
        else
        {
            await StartWithShowRewardsList().FastAwait();
        }
    }

    private async Task StartWithClaimNewReward()
    {
        var sb = new StringBuilder()
            .AppendLine(Localization.Get(session, "dialog_daily_bonus_event_header").Preformatted())
            .AppendLine()
    ...
    private async Task StartWithShowRewardsList()
    {
        var sb = new StringBuilder()
