...
public class PlayerResources
{
    ...
    public bool TryPurchase(IEnumerable<ResourceData> requiredResources)
    {
        ...
        foreach (var (resourceId, amount) in requiredResources)
        {
            ...
            Program.logger.Info($"RESOURCE SPENDING | {_session.actualUser} : {resourceId} ({amount})");
        }
        return true;
    }
    ...
    public bool TryPurchase(ResourceData requiredResource)
    {
        ...
        if (success)
        {
            ...
            Program.logger.Info($"RESOURCE SPENDING | {_session.actualUser} : {requiredResource.resourceId} ({requiredResource.amount})");
        }
        return success;
    }
